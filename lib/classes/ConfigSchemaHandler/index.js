...
class ConfigSchemaHandler {
  ...
  defineFunctionProperties(providerName, configSchema) {
    if (arguments.length === 1) {
      configSchema = providerName;
      providerName = null;
    } else {
      providerName = Array.isArray(providerName) ? providerName : [providerName];
    }

    if (providerName && !providerName.includes(this.serverless.service.provider.name)) {
      return;
    }
